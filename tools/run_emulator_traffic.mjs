...
const trafficCommand = [ 'node tools/traffic_runner.mjs', forwardedArgs ].filter( Boolean ).join( ' ' )
const command = `npx firebase-tools emulators:exec --project demo-qt4-e2e --only auth,firestore,storage "node tools/seed_emulators.mjs && ${trafficCommand}"`
